...
class Importer:
	...
	def __loadEntity(self, raw_entity, entity_type, depth=0):
		...
		try:
			raw_entity["entity_type"] = entity_type
			storage = self.getEntityList(entity_type)
			new_entity = sw5e.Entity.Entity(raw_entity, importer=self)
			uid = new_entity.uid

			if old_entity := storage.get(uid):
				if utils.object.getProperty(raw_entity, 'propertiesMap.Modal'): return
			...
			for uid, entity in storage.items():
				...
				if len(entity.broken_links) or not entity.processed:
					try:
						...
						print(f'		{uid=} {depth=} {entity.__class__=}')
						raise
	...
	def getUID(self, entity_type, raw_entity):
		return sw5e.Entity.Entity.getUID(raw_entity, entity_type=entity_type)
